...
public class Game {
	...
	public static final Cell[] cells = {
		...
		new Cell("成功湖",370,310,CellType.SELECT).setScore(20).setMessage("被校狗追"),
		...
		new Cell("「飛撲」雕塑",465,690,CellType.SELECT).setScore(-100).setMessage("穿越飛撲，微積分被當").setSelectPolicy(PlayerScoreType.LESSON),
		new Cell("總圖書館",560,690,CellType.FATE).setScore(1).setMessage("知識王"),
		new Cell("資訊系館",655,690,CellType.SELECT).setScore(20).setMessage("認真上課"),
		new Cell("D24",750,690,CellType.CHANCE),
		new Cell("九乘九",845,690,CellType.HOSPITAL).setMessage("住院一週，暫停兩回合"),
		new Cell("育樂街",940,690,CellType.SHOP).setMessage("交易時間"),
		...
		new Cell("麥當勞",1225,590,CellType.SELECT).setScore(200).setMessage("五秒內吃完一個大麥克"),
